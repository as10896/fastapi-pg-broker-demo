...
from app.broker import control, messages, registry
...
SNIPPETS = [
    ...
    Snippet(
        ...
        registry.WORKER_HEARTBEAT_SQL
        + "\n-- then, for each consumer:\n"
        + registry.CONSUMER_HEARTBEAT_SQL
        + "\n-- then, for every message still being processed:\n"
        + messages.RENEW_LEASE_SQL,
...
async def _registry(pool: Pool) -> dict[str, Any]:
    workers = await registry.list_workers(pool)
    consumers = await registry.list_consumers(pool)
...
async def start_consumers(form: Annotated[StartConsumersForm, Form()], pool: PoolDep):
    sent = await control.send_command(
...
async def stop_all_consumers(worker_id: str, pool: PoolDep):
    if not await control.send_command(pool, worker_id, "stop"):
...
async def stop_consumer(worker_id: str, consumer_id: str, pool: PoolDep):
    if not await control.send_command(pool, worker_id, "stop", consumer_id=consumer_id):
        return _gone(worker_id)
...
async def kill_consumer(worker_id: str, consumer_id: str, pool: PoolDep):
    consumer = await registry.get_consumer(pool, consumer_id)
    if not await control.send_command(pool, worker_id, "kill", consumer_id=consumer_id):
        return _gone(worker_id)
